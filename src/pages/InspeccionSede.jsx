...
export default function InspeccionSede() {
    ...
    <>
      <button onClick={limpiar}
        className="bg-gray-100 border border-border text-muted text-[0.78rem] font-bold
                   uppercase tracking-wide px-4 py-2 rounded-btn hover:bg-red-50
                   hover:border-red-300 hover:text-red-400 transition-all">
        ↺ Limpiar
      </button>
      <button onClick={guardar} disabled={saving}
        className="gradient-green text-white font-extrabold text-[0.82rem] px-5 py-2 rounded-btn
  ...
  return (
    <AppLayout titulo="Inspección de Sede" acciones={acciones}>
    
          {/* ── Card: Datos de la inspección ── */}
          <div className="bg-card border border-border rounded-card shadow-sm p-6 mb-5">
            <div className="flex items-center gap-2 mb-5">
              <div className="card-title-bar" />
              <span className="text-[0.7rem] font-bold uppercase tracking-[2px] text-accent1">Datos de la inspección</span>
            </div>
            ...
            <div className="bg-card border border-border rounded-card shadow-sm p-6">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea value={form.observacion} onChange={e => setField('observacion', e.target.value)}
                          placeholder="Observación..." rows={4} className={`${inputCls} resize-y`} />
              </div>
            </div>
              ...
              <div className="bg-card border border-border rounded-card shadow-sm p-6">
                ...
                <div className="flex flex-col gap-2.5">
                  {[
                    ...
                    <div key={item.key}
                      className="flex items-center justify-between px-4 py-3 border border-accent1/10 rounded-xl hover:bg-blue-50 transition-colors">
                      <div className="flex items-center gap-2.5 text-[0.88rem] font-medium text-ink">
                        <div className="w-8 h-8 rounded-lg grid place-items-center flex-shrink-0" style={{ background: item.bgColor }}>
                          <svg className="w-4 h-4 fill-none" strokeWidth="2" style={{ stroke: item.iconColor }} viewBox="0 0 24 24">
                            <rect x="3" y="3" width="18" height="18" rx="2"/>
                          </svg>
                        </div>
                        {item.label}
                      </div>
                      <Toggle checked={control[item.key]} onChange={val => setControl(c => ({ ...c, [item.key]: val }))} />
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-card border border-border rounded-card shadow-sm p-6 mb-5">
            ...
            {esEstandar ? (
              <div className="grid grid-cols-3 gap-4">
                <ImageBox label="A. Principal" image={imgs.aPrincipal}
                  onImage={v => setImgs(i => ({ ...i, aPrincipal: v }))}
                  onClear={() => setImgs(i => ({ ...i, aPrincipal: null }))} />
                <ImageBox label="A. Soporte" image={imgs.aSoporte}
                  onImage={v => setImgs(i => ({ ...i, aSoporte: v }))}
                  onClear={() => setImgs(i => ({ ...i, aSoporte: null }))} />
                <ImageBox label={form.dvrCamara ? `CCTV: ${form.dvrCamara.toUpperCase()}` : 'CCTV: —'}
                  image={imgs.cctv}
                ...
                {extras.map(ex => (
                  <div key={ex.id} className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <input value={ex.nombre} onChange={e => actualizarExtra(ex.id, 'nombre', e.target.value)}
                        className="bg-transparent border-b border-border text-[0.78rem] font-semibold text-ink outline-none px-1 py-0.5 w-40 focus:border-accent1" />
                      <button onClick={() => eliminarExtra(ex.id)}
                        className="text-[0.72rem] font-bold text-red-400 hover:text-red-600 transition-colors">✕ Eliminar</button>
                    </div>
                    <ImageBox image={ex.imagen} onImage={v => actualizarExtra(ex.id, 'imagen', v)} onClear={() => actualizarExtra(ex.id, 'imagen', null)} />
                  </div>
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-3 gap-4">
                <ImageBox label={form.dvrCamara ? form.dvrCamara.toUpperCase() : 'DVR — Cámara'}
                  image={imgs.default} height="200px"
